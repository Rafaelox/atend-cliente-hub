...
const ChatMessage = ({ sender, message, timestamp, clientName }: ChatMessageProps) => {
  ...
  const getSenderStyle = (sender: string) => {
    switch (sender) {
      ...
      case 'atendente':
        return 'bg-gradient-to-r from-green-50 to-green-100 text-green-900 border-l-4 border-l-green-500 ml-4 lg:ml-8';
      default:
  ...
  return (
    <div className={`p-3 lg:p-4 rounded-lg lg:rounded-xl shadow-sm ${getSenderStyle(sender)}`}>
      ...
      <p className="text-sm lg:text-sm leading-relaxed">{message}</p>
    </div>
  );
};
